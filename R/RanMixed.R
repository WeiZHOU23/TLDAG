########################################
### TLDAG for the random mixed graph ###
########################################
...
#################################################
Kappa.cal <- function(KC.layer, KC.X, KC.n, KC.p, KC.beta1.pois, KC.beta2.pois, KC.beta1.bino, KC.beta2.bino, KC.N, KC.seed){
...
Kappa <- function(Ka.layer, Ka.X, Ka.grid.t, Ka.beta1.pois, Ka.beta2.pois, Ka.beta1.bino, Ka.beta2.bino, Ka.N){
  ...
  Kappa.temp<-sapply(c(1,2,3,4,5), Kappa.cal, KC.layer=Ka.layer, KC.X=Ka.X, KC.n=Ka.n, KC.p=Ka.p, 
                     KC.beta1.pois=Ka.beta1.pois, KC.beta2.pois=Ka.beta2.pois, KC.beta1.bino=Ka.beta1.bino, KC.beta2.bino=Ka.beta2.bino, KC.N=Ka.N)
...
####################################################
## get the index of binomial node in the mixed DAG
####################################################
    ...
    for (k1 in 1:length(left.order) ) { # k 代表原始的变量
      ...
      if(length(C[[k]]) ==0 ){  
        ...
        if(k %in% IndexBino){
          ...
          S[kk] <- RatioODS(as.matrix(X[, k]), beta_1 = beta2.bino, beta_2 = beta2.pois)
        }
...
do_one <- function(do.n, do.p, do.N, do.grid.t, do.T, do.graph=c('sparse','dense'), do.theta1.pois, do.theta2.pois, do.beta1.pois, 
                   do.beta2.pois, do.theta1.bino, do.theta2.bino, do.seed){
  do.result <- matrix(0, 6, 5) ##
  # Data generation
  do.dat <- get_DAGdata(do.n, do.p, do.T, do.graph, do.theta1.pois, do.theta2.pois, 
                        do.beta1.pois, do.beta2.pois, do.theta1.bino, do.theta2.bino, do.N, do.seed) # N is the size for binomial
  do.truth <- do.dat$DAG
  do.X <- do.dat$X
...
simu_replicate <- function(simr.n, simr.p, simr.times, simr.grid.t, simr.T, simr.graph=c('sparse','dense'), simr.theta1.pois, simr.theta2.pois, simr.beta1.pois, simr.beta2.pois, simr.theta1.bino, simr.theta2.bino, simr.N){
  simr.results <- list()
  ...
  for (i in 1:simr.times) {
    ...
    simr.results[[i]] <- do_one(simr.n, simr.p, simr.N, simr.grid.t, simr.T, simr.graph, simr.theta1.pois, simr.theta2.pois, simr.beta1.pois, simr.beta2.pois, simr.theta1.bino, simr.theta2.bino, i)$Perf
  }                   
...
###################################################
## generate data for random graph for mixed DAG  
###################################################


randomMixedDag <- function(RBD.p, RBD.T, RBD.graph=c('dense','sparse'), RBD.seed){
  ...
  RBD.layer <- list()
  set.seed(RBD.seed)
  RBD.layer[1] <- sample(RBD.p - RBD.numberLayer, 1, replace = FALSE)
  for (i in 2:RBD.T){
    RBD.done <- sum(unlist(RBD.layer))
    RBD.left <- RBD.numberLayer - length(RBD.layer) 
...
get_DAGdata <- function(getD.n, getD.p,getD.T, getD.graph=c('sparse','dense'), getD.theta1.pois, getD.theta2.pois, getD.beta1.pois, getD.beta2.pois, getD.theta1.bino, getD.theta2.bino, getD.N, getD.seed){
  # N is size for binomial parameter
  set.seed(getD.seed)
  getD.dat <- randomMixedDag(getD.p, getD.T, getD.graph, getD.seed)
  ...
  set.seed(getD.seed)
  getD.theta.bino <- runif(getD.p, getD.theta1.bino, getD.theta2.bino)
  set.seed(getD.seed)
  getD.beta.bino  <- matrix(runif(getD.p*getD.p, getD.theta1.bino, getD.theta2.bino), getD.p, getD.p)
  
  set.seed(getD.seed)
  getD.theta.pois <- runif(getD.p, getD.theta1.pois, getD.theta2.pois)
  set.seed(getD.seed)
  getD.beta.pois  <- matrix(runif(getD.p*getD.p, getD.beta1.pois, getD.beta2.pois), getD.p, getD.p)
  
  getD.X <- matrix(0, getD.n, getD.p)
  getD.Index <- list()
  
  ## generate data for the roots
  get.A0.node <- getD.nodes[[1]]
  get.A0.length <- length(get.A0.node)
  set.seed(getD.seed)
  get.A0.index <- sample(2, get.A0.length, replace = TRUE)
  getD.Index[[1]] <- get.A0.index
  
  for (root in 1:get.A0.length) {
    
    A0.root.index <- get.A0.node[root]
    if(get.A0.index[root] == 1){
      # poisson root 
      get.A0.lambda <- exp(getD.theta.pois[A0.root.index])
      set.seed(getD.seed)
      getD.X[, A0.root.index] <- rpois(getD.n, lambda = get.A0.lambda)
      
    }else{
      # binomial root
      set.seed(getD.seed)
      get.A0.prob <- exp(getD.theta.bino[A0.root.index]) / (1 + exp(getD.theta.bino[A0.root.index]))
      getD.X[, A0.root.index] <- rbinom(getD.n, size = getD.N, prob = get.A0.prob)
      
    }
  }
  
  ## generate data for At layer
  for (k in 2:(getD.T+1)) {
    get.At.node <- getD.nodes[[k]]
    get.At.length <- length(get.At.node)
    set.seed(getD.seed)
    get.At.index <- sample(2, get.At.length, replace = TRUE)
    getD.Index[[k]] <- get.At.index
    
    for (node in 1:get.At.length) {
      
      At.node.index <- get.At.node[node]
      At.parent <- which(getD.DAG[, At.node.index] != 0)
      
      if(get.At.index[node] == 1){
        # poisson root 
        if(length(At.parent) == 1){
          set.seed(getD.seed)
          get.At.lambda <- exp(getD.theta.pois[At.node.index] + getD.X[, At.parent] * getD.beta.pois[At.node.index, At.parent])
          getD.X[, At.node.index] <- rpois(getD.n, lambda = get.At.lambda)
        ...
        # binomial root
        if(length(At.parent) == 1){
          At.deno <- exp(getD.theta.bino[At.node.index] + getD.X[, At.parent] * getD.beta.bino[At.node.index, At.parent])
          At.prob <- At.deno / (1+At.deno)
          set.seed(getD.seed)
          getD.X[, At.node.index] <- rbinom(getD.n, size = getD.N, prob = At.prob)
  ...
  Index.matrix <- matrix(0, getD.p, 2)
  Index.matrix[, 1] <- unlist(getD.nodes)
  Index.matrix[, 2] <- unlist(getD.Index)
